<template>
  <aside class="bg-slate-100 px-3">
    ...
    <div class="w-[100%] py-[10px] flex justify-between">
      <div
        class="search w-[33%] flex border-green rounded-md border-[2px] justify-between p-3 bg-white"
      >
        <div class="searchtext flex gap-4 items-center justify-around">
          ...
          >
            <n-button
              class="bx bx-chevron-down text-[25px] text-slate-400 cursor-pointer"
            ></n-button>
          </n-dropdown>
        ...
        <input
          v-model="productName.name"
          class="h-7 outline-none searchinput"
          type="text"
          :placeholder="t('appbar.search')"
        />
        <i
          @click="searchProducts"
          class="bx bx-search-alt-2 text-xl text-slate-400 cursor-pointer"
        ></i>
      </div>
      ...
      <div class="flex items-center gap-14 justify-between top3">
        ...
        <div class="flex items-center gap-2">
          ...
          <router-link to="/wishlist"
            ><span class="text-slate-500 text-[15px]">{{ t('appbar.wishlist') }}</span>
          </router-link>
        </div>
        ...
        >
          <div
            class="relative"
            :class="getProduct?.length ? 'mr-2' : 'mr-1'"
          >
        ...
        <div class="flex items-center gap-2">
          ...
          <router-link to="/account"
            ><span class="text-slate-500 text-[15px]">{{ t('appbar.account') }}</span>
          </router-link>
        </div>
      </div>
    </div>
...
async function handleSelect(key: number) {
  ...
  const filteredProducts = productlist.value.filter(
    (product) => product.category.category_id === key
  );
