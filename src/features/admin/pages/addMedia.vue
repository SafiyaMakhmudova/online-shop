...
const product_id = addedProoduct && addedProoduct.value.find((item) => item.id);
const media = reactive<mediaObj>({
  product_id: product_id ? Number(product_id.id) : undefined,
  media_link: ''
});

async function handleClick() {
  if (!media.product_id || !media.media_link) {
