<template>
  ...
  <div v-else>
    ...
    <div v-show="showUpdate" class="absolute bg-primary p-5 w-[50%] ml-[10%] mr-[10%] mt-[10%] top-14  rounded-xl">
        <div class="flex justify-between">
          <p class="text-lg  w-[40%] flex items-center  ">
            First Name :
            <input class="p-1 outline-none rounded-lg ml-2 mt-2  w-[50%]" v-model="singleAdmin.first_name" type="text" name="name" id="" />
          </p>
          <i @click="canceled" class="bx bx-x bx-black bx-lg hover:bg-red-600 rounded-lg"></i>
        </div>
        <p class="text-lg w-[40%] flex items-center">
          Last Name :
          <input class="p-1  w-[50%] outline-none rounded-lg ml-4 mt-2" v-model="singleAdmin.last_name" type="text"
            name="description" id="" />
        </p>
        <p class="text-lg w-[40%] flex items-center">
          Address :
          <input class="p-1  w-[50%] outline-none rounded-lg ml-8 mt-2" v-model="singleAdmin.address" type="text"
            name="description" id="" />
        </p>
        <p class="text-lg w-[40%] flex items-center">
          Password :
          <input class="p-1  w-[50%] outline-none rounded-lg ml-5 mt-2" v-model="singleAdmin.password" type="text"
            name="description" id="" required="true" />
        </p>
        <p class="text-lg w-[40%] flex items-center">
          Super Admin :
            <input
            class="p-1  w-[10%] outline-none rounded-lg " v-model="singleAdmin.is_superAdmin"
            type="checkbox"
            required="true"
          />
        </p>
        <button @click="saved" class="p-2 mt-10  bg-orange w-[100px] rounded-lg hover:bg-blue-600 hover:text-white">
          Saved
        </button>
      </div>
  </div>
...
const { singleAdmin, loading, route, fetchsingleAdmin, router, fetchUpdateAdmin, removeAdmin } = useAdmin();
...
async function saved() {
  ...
  if(!updateAdmin.address && !updateAdmin.first_name && updateAdmin.last_name && updateAdmin.password ) {
